...
void VanitySearch::getGPUStartingKeys(int thId, int groupSize, int nbThread, Point itargetPubKey, Int *keys, Point *p) {
  ...
  int trbit = pow2W - tl;// Tame pow2W-0 ?
  int wrbit = pow2W - wl;// Wild pow2W-1 ?
  ...
  for (int g = 0; g < GPU_GRP_SIZE; g++) {
	...
	if (rekey > 1) {
		// Get keys
		keys[i*GPU_GRP_SIZE + g].SetInt32(0);
		tkey.Rand(trbit);
		wkey.Rand(wrbit);		
		if (g % 2 == TAME) {
			keys[i*GPU_GRP_SIZE + g].Add(&TameStartingKey);// Tame keys
			keys[i*GPU_GRP_SIZE + g].Add(&tkey);
			// Check GPU Random Key
			if (g < 10 && i == 0) printf("\nGPU Tame Starting Key %d: %s ", g, keys[i*GPU_GRP_SIZE + g].GetBase16().c_str());
			if (g == GPU_GRP_SIZE-1 && i == nbThread-1) printf("\nGPU Tame Starting Key %d: %s Kangaroos: %d ", (i*g)+g, keys[i*GPU_GRP_SIZE + g].GetBase16().c_str(), nbThread*GPU_GRP_SIZE);
		} else {
			keys[i*GPU_GRP_SIZE + g].Add(&wkey);// Wild keys
			// Check GPU Random Key
			if (g < 10 && i == 0) printf("\nGPU Wild Starting Key %d: %s ", g, keys[i*GPU_GRP_SIZE + g].GetBase16().c_str());
			if (g == GPU_GRP_SIZE-1 && i == nbThread-1) printf("\nGPU Wild Starting Key %d: %s Kangaroos: %d ", (i*g)+g, keys[i*GPU_GRP_SIZE + g].GetBase16().c_str(), nbThread*GPU_GRP_SIZE);
		}
	} else {
      // Get keys
	  keys[i*GPU_GRP_SIZE + g].SetInt32(0);
    ...
    // For check GPU code
	//keys[i*GPU_GRP_SIZE + g].SetBase16("FF");
	//keys[i*GPU_GRP_SIZE + g].SetInt32(10);
	
	Int k(keys[i*GPU_GRP_SIZE + g]);
    p[i*GPU_GRP_SIZE + g] = secp->ComputePublicKey(&k);
	if (g % 2 == WILD) { // Set 0 for check compute points in GPU
	//if (0) {
		p[i*GPU_GRP_SIZE + g] = secp->AddAffine(itargetPubKey, p[i*GPU_GRP_SIZE + g]);
	}
  }
  }
...
void VanitySearch::CreateJumpTable() {
	...
	// Create Jump Table
	Point *Sp = new Point[NB_JUMP];// !!! Set in GPUEngine.h #define NB_JUMP 64
...
void VanitySearch::CreateJumpTable(uint32_t Jmax, int pow2w) {
	...
	*/
	// Add small jumps
	int small_jump = NB_JUMP / 2;// #define NB_JUMP 32// Do not change!
...
void VanitySearch::FindKeyGPU(TH_PARAM *ph) {
  ...
  GPUJmaxofSp = (uint32_t)pow2Wsqrt / 2;
  ...
  printf("GPU %s (%.1f MB used; GRP size: %d) \n",g.deviceName.c_str(), g.GetMemory() / 1048576.0, (int)GPU_GRP_SIZE);
  ...
  while (ok && !flag_endOfSearch) {
	...
	for(int i=0;i<(int)found.size() && !flag_endOfSearch;++i) {
		...
		if (1) {
			...
			file_cnt = n_count + (uint32_t)found.size();
			
			nb_work = file_cnt % NB_WORK;
			
			f2save = File2save(&Tpx, &Tkey, kType, nb_work);
